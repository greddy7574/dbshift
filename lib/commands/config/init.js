...
async function configInitCommand(options) {
  ...
  try {
    ...
    // 检查是否是来自对话框的结果
    if (options.configType && options.dbConfig) {
      // 来自对话框的配置
      const { configType, dbConfig, useDefaults } = options;
      
      console.log(chalk.green(`✓ Using ${configType === 'env' ? '.env' : 'schema.config.js'} format`));
      console.log(chalk.gray(`  Host: ${dbConfig.host}:${dbConfig.port}`));
      console.log(chalk.gray(`  User: ${dbConfig.user}`));
      console.log(chalk.gray(`  Password: ${dbConfig.password ? '***' : 'N/A'}`));
...
`;
  ...
  production: {
    ...
    user: process.env.MYSQL_USERNAME || '${dbConfig.user}',
