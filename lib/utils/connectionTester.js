...
class ConnectionTester {
  ...
  static async testConnection(dbConfig, options = {}) {
    ...
    try {
      ...
      const [result] = await database.connection.query('SELECT 1 as test_connection, VERSION() as mysql_version');
      const queryTime = Date.now() - queryStartTime;
      
      // 获取服务器信息
      const [serverInfo] = await database.connection.query('SELECT @@version_comment as server_comment');
  ...
  static async testMigrationTableAccess(dbConfig, verbose = true) {
    ...
    try {
      // 检查迁移表是否存在
      const [tableExists] = await database.connection.query(`
      ...
      if (migrationResult.table_exists) {
        // 获取迁移统计信息
        const [migrationStats] = await database.connection.query(`
          SELECT 
